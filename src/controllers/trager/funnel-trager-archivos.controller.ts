...
export const listarArchivosFunnelTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const data = await tragerPrisma.funnelTragerArchivo.findMany({
      where: { oportunidadId },
    ...
    console.error('Error al listar archivos Funnel Trager:', error);
    res.status(500).json({ success: false, error: 'Error interno del servidor' });
  }
};

export const subirArchivosFunnelTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    const oportunidadId = parseIdParam(req.params.id);
    if (!oportunidadId) {
      res.status(400).json({ success: false, error: 'ID invalido' });
    ...
    const folder = `trager/oportunidades/${oportunidadId}`;

    const data = await Promise.all(
      files.map(async (file) => {
        const result = await uploadFileDetailed(file.buffer, folder);
        return tragerPrisma.funnelTragerArchivo.create({
          data: {
            oportunidadId,
            tipo,
    ...
    console.error('Error al subir archivos Funnel Trager:', error);
    res.status(500).json({ success: false, error: 'Error interno del servidor' });
  }
};

export const eliminarArchivoFunnelTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const archivo = await tragerPrisma.funnelTragerArchivo.findUnique({
      where: { id: archivoId },
    });
    ...
    await tragerPrisma.funnelTragerArchivo.delete({
      where: { id: archivoId },
    });

    res.json({ success: true, message: 'Archivo eliminado' });
  } catch (error) {
    console.error('Error al eliminar archivo Funnel Trager:', error);
    res.status(500).json({ success: false, error: 'Error interno del servidor' });
  }
};
