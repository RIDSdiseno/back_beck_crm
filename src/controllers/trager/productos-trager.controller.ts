...
const CLOUDINARY_FOLDER = 'Trager/productos';
...
const extractPublicId = (url: string): string | null => {
  try {
    ...
    const folderIdx = withoutExt.indexOf('Trager/productos');
...
export const getProductosTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.error('Error al obtener productos Trager:', error);
    res.status(500).json({ success: false, error: 'Error al obtener productos' });
  }
};

export const getProductoTragerById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const producto = await tragerPrisma.producto.findUnique({
      where: { id },
      include: { Categoria: true },
    });
    ...
    console.error('Error al obtener producto Trager:', error);
    res.status(500).json({ success: false, error: 'Error al obtener producto' });
  }
};

export const createProductoTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    const {
      nombre,
      sku,
    ...
    const cat = await tragerPrisma.categoria.findUnique({ where: { id: catId } });
    if (!cat) {
    ...
    const producto = await tragerPrisma.producto.create({
      data: {
        nombre: nombre.trim(),
        sku: sku.trim(),
    ...
    console.error('Error al crear producto Trager:', error);
    res.status(500).json({ success: false, error: 'Error al crear producto' });
  }
};

export const updateProductoTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseIdParam(req.params.id);
    if (!id) {
      res.status(400).json({ success: false, error: 'ID inválido' });
    ...
    const existing = await tragerPrisma.producto.findUnique({ where: { id } });
    if (!existing) {
      res.status(404).json({ success: false, error: 'Producto no encontrado' });
      return;
    }
    ...
    if ('stock' in req.body || 'stockActual' in req.body || 'stock_actual' in req.body) {
      res.status(400).json({
        success: false,
        error: 'stockActual no se puede modificar directamente desde el CRUD de productos.',
      });
      return;
    }
    ...
    if (sku !== undefined) {
      ...
      const dup = await tragerPrisma.producto.findFirst({
    ...
    const updated = await tragerPrisma.producto.update({
      where: { id },
      data,
      include: { Categoria: true },
    });
    ...
    console.error('Error al actualizar producto Trager:', error);
    res.status(500).json({ success: false, error: 'Error al actualizar producto' });
  }
};

export const asignarCategoriaProductosTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const categoria = await tragerPrisma.categoria.findUnique({ where: { id: catId } });
    if (!categoria) {
      res.status(404).json({ success: false, error: 'Categoría no encontrada' });
      return;
    }

    const productosExistentes = await tragerPrisma.producto.count({
      where: { id: { in: ids } },
    });
    ...
    const result = await tragerPrisma.producto.updateMany({
      where: { id: { in: ids } },
    ...
    console.error('Error al asignar categoría masiva Trager:', error);
    res.status(500).json({ success: false, error: 'Error al asignar categoría' });
  }
};

export const patchEstadoProductoTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseIdParam(req.params.id);
    if (!id) {
      res.status(400).json({ success: false, error: 'ID inválido' });
    ...
    console.error('Error al cambiar estado producto Trager:', error);
