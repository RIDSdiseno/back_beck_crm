...
type EtapaFunnelTrager = (typeof ETAPAS_PERMITIDAS)[number];
...
const parseEtapa = (value: unknown): EtapaFunnelTrager | null => {
  if (typeof value !== 'string') return null;
  return ETAPAS_PERMITIDAS.includes(value as EtapaFunnelTrager)
    ? (value as EtapaFunnelTrager)
    : null;
};
...
interface CamposSnapshotTrager {
  cliente: string | null;
...
async function ejecutarValidacionDinamicaTrager(
  _etapaParaReglas: string,
  etapaDestino: string,
  c: CamposSnapshotTrager
): Promise<{ bloqueos: string[]; advertencias: string[]; puedeAvanzar: boolean; reglasCargadas: string[] }> {
  const mapa = await obtenerMapaReglasValidacion('TRAGER', etapaDestino);
...
const snapshotDesdeBody = (body: Record<string, unknown>): CamposSnapshotTrager => ({
...
const snapshotDesdeCurrent = (
  current: Prisma.FunnelTragerOpportunityGetPayload<Record<string, never>>,
  body: Record<string, unknown>
): CamposSnapshotTrager => ({
...
const handleError = (res: Response, error: unknown): void => {
  ...
  console.error('Error en Funnel Trager:', error);
...
const registrarCambioEtapaTrager = async ({
  tx,
  oportunidadId,
  etapaAnterior,
  etapaNueva,
  ...
  await tx.historialEtapaTrager.create({
...
const cargarClientesParaMatch = async (): Promise<ClienteRegistradoSets> => {
  const clientes = await tragerPrisma.cliente.findMany({
    where: { activo: true },
    select: { rut: true, contactoCorreo: true, contactoTelefono: true },
  });
  return {
    ruts: new Set(clientes.map(c => c.rut).filter((r): r is string => r !== null)),
    emails: new Set(clientes.map(c => c.contactoCorreo?.toLowerCase()).filter((e): e is string => e !== null)),
    telefonos: new Set(clientes.map(c => c.contactoTelefono).filter((t): t is string => t !== null)),
  };
};
...
const buildCreateData = (body: Record<string, unknown>): Prisma.FunnelTragerOpportunityUncheckedCreateInput => {
...
const buildUpdateData = (
  current: Prisma.FunnelTragerOpportunityGetPayload<Record<string, never>>,
  body: Record<string, unknown>
): Prisma.FunnelTragerOpportunityUncheckedUpdateInput => {
  const etapa = hasOwn(body, 'etapa') ? parseEtapa(body.etapa) : (current.etapa as EtapaFunnelTrager);
  if (!etapa) {
  ...
  const data: Prisma.FunnelTragerOpportunityUncheckedUpdateInput = {};
...
export const getFunnelTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    const { q, etapa, responsable, cliente, cotizacionId } = req.query;
    const where: Prisma.FunnelTragerOpportunityWhereInput = {};
    ...
    const data = await tragerPrisma.funnelTragerOpportunity.findMany({
      where,
...
export const getFunnelTragerById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const data = await tragerPrisma.funnelTragerOpportunity.findUnique({
      where: { id },
    });
...
export const createFunnelTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!observacionCrear) {
      const validacion = await ejecutarValidacionDinamicaTrager(etapa, etapa, snapshot);
    ...
    const data = await tragerPrisma.$transaction(async (tx) => {
      const created = await tx.funnelTragerOpportunity.create({
        data: { ...dataInput, fechaUltimoCambioEtapa: new Date() },
      });
      await registrarCambioEtapaTrager({
        tx,
    ...
    res.status(201).json({
      ...
      message: 'Oportunidad Trager creada',
...
export const updateFunnelTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseIdParam(req.params.id);
    if (!id) {
      res.status(400).json({ success: false, error: 'ID invalido' });
    ...
    const current = await tragerPrisma.funnelTragerOpportunity.findUnique({ where: { id } });
    if (!current) {
      res.status(404).json({ success: false, error: 'Oportunidad no encontrada' });
      return;
    }
    ...
    if (nuevaEtapa !== null && nuevaEtapa !== current.etapa) {
      ...
      if (!esRetrocesoUpd) {
        ...
        if (!observacionFinalUpd) {
          ...
          const validacion = await ejecutarValidacionDinamicaTrager(current.etapa, nuevaEtapa, snapshot);
    ...
    const data = await tragerPrisma.$transaction(async (tx) => {
      const updated = await tx.funnelTragerOpportunity.update({
        where: { id },
        data: dataInput,
      });
    ...
    res.json({
      success: true,
      data: { ...data, clienteRegistrado: esClienteRegistrado(data, clienteSets) },
      message: 'Oportunidad Trager actualizada',
...
export const patchEtapaFunnelTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseIdParam(req.params.id);
    if (!id) {
      res.status(400).json({ success: false, error: 'ID invalido' });
    ...
    if (etapa !== current.etapa) {
      ...
      if (!esRetroceso) {
        ...
        if (!observacionFinalPatch) {
          const snapshot = snapshotDesdeCurrent(current, body);
          const validacion = await ejecutarValidacionDinamicaTrager(current.etapa, etapa, snapshot);
...
export const deleteFunnelTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseIdParam(req.params.id);
    if (!id) {
      res.status(400).json({ success: false, error: 'ID invalido' });
    ...
    await tragerPrisma.funnelTragerOpportunity.delete({ where: { id } });

    res.json({ success: true, message: 'Oportunidad Trager eliminada' });
  } catch (error) {
    handleError(res, error);
  }
};

export const getHistorialEtapasTrager = async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseIdParam(req.params.id);
    if (!id) {
      res.status(400).json({ success: false, error: 'ID invalido' });
    ...
    const oportunidad = await tragerPrisma.funnelTragerOpportunity.findUnique({
      where: { id },
      select: { id: true },
    });
    if (!oportunidad) {
    ...
    const historial = await tragerPrisma.historialEtapaTrager.findMany({
