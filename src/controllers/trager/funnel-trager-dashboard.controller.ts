...
type EtapaTrager = (typeof ETAPAS_TODAS)[number];
...
const ETAPAS_FUNNEL: EtapaTrager[] = [
  'PROSPECTO',
  'PRIMER_CONTACTO',
  'DESARROLLO_COTIZACION',
  'COTIZACION_ENVIADA',
...
const ETAPA_LABELS: Record<EtapaTrager, string> = {
  PROSPECTO:              'Prospecto',
...
export async function getDashboardFunnelTrager(req: Request, res: Response): Promise<void> {
  try {
    const {
      unidadNegocio,
      responsable,
    ...
    const opps: OppRow[] = await tragerPrisma.funnelTragerOpportunity.findMany({
      where,
      select: {
        id:                 true,
        cliente:            true,
    ...
    const tasaRecompraTrager = totalOportunidades > 0
    ...
    const whereSinSeg: Prisma.FunnelTragerOpportunityWhereInput = {
      ...where,
    ...
    const [sinSegList, totalSinSeguimiento] = await Promise.all([
      tragerPrisma.funnelTragerOpportunity.findMany({
        where: whereSinSeg,
      ...
      tragerPrisma.funnelTragerOpportunity.count({ where: whereSinSeg }),
    ]);
    ...
    console.error('Error en getDashboardFunnelTrager:', error);
